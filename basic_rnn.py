...
class BasicRNN(object):
    ...
    def save_model_parameters(self):
        time = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        with open("./data/rnn-w2v-%d-%d-%s.pkl" % (self.hidden_dim, self.in_dim, time), "wb") as model_file:
    ...
    def load_model_parameters(self, path_to_file):
        ...
        # self.in_dim = self.U.shape[1]
        # self.out_dim = self.V.shape[0]

        # print "Loaded model parameters from %s. hidden_dim=%d in_dim=%d out_dim=%d" % \
        #       (path_to_file, self.hidden_dim, self.in_dim, self.out_dim)
