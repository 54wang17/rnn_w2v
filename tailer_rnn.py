...
class RNN(object):
    ...
    """
    def __init__(self, x, n_in, n_hidden, n_out, activation=T.tanh):
        ...
        U_init = np.asarray(np.random.uniform(size=(n_in, n_hidden),
                                                 low=-.01, high=.01),
                                                 dtype=theano.config.floatX)
        self.U = theano.shared(value=U_init, name='W_in')

        s0_init = np.zeros((n_hidden,), dtype=theano.config.floatX)
        self.s0 = theano.shared(value=s0_init, name='h0')

        bh_init = np.zeros((n_hidden,), dtype=theano.config.floatX)
        self.bh = theano.shared(value=bh_init, name='bh')

        self.params = [self.W, self.U, self.s0, self.bh]
        ...
        # activation function
        def step(x_t, s_tm1):
            s_t = self.activation(T.dot(x_t, self.U) + \
                                  T.dot(s_tm1, self.W) + self.bh)
            return s_t
        ...
        prediction = T.argmax(self.p_y_given_x, axis=1)

        # compute prediction as class whose probability is maximal
        self.loss = lambda y : T.nnet.categorical_crossentropy(self.p_y_given_x, y)
